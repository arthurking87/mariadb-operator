...
func (r *EndpointsReconciler) endpointSlice(ctx context.Context, key types.NamespacedName,
	...
	// Every secondary Pod that passes buildEndpoint validation gets an Endpoint entry
	// regardless of readiness — buildEndpoint sets Conditions.Ready per-Pod but never
	// drops the entry on its own. A Pod missing PodIP or NodeName fails validation and is
	// skipped below instead. We don't remove not-ready Pods from the EndpointSlice
	// ourselves; we rely on kube-proxy (and other EndpointSlice consumers) honoring
	// Ready=false and excluding those addresses from regular traffic.
